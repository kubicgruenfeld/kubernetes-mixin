    ...
    [if $._config.alerts.kubelet_enabled then 'groups']+: [
      {
        ...
        rules: [
          ...
          },
          {
            ...
            expr: |||
              histogram_quantile(0.99, sum(rate(kubelet_pod_worker_duration_seconds_bucket{%(kubeletSelector)s}[5m])) by (instance, le, %(clusterLabel)s)) * on(instance, %(clusterLabel)s) group_left(node,%(clusterGroupLabelsStr)s) kubelet_node_name{%(kubeletSelector)s} > 60
            ||| % $._config,
            'for': '15m',
            labels: {
              severity: 'warning',
